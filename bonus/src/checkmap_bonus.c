...
/*   Updated: 2025/11/08 09:37:09 by lbardet-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checkmandatories2(t_data *data)
{
	...
	while (data->map[data->i])
	{
		if (data->map[data->i] == 'L')
			data->countcheck += 1;
		data->i++;
	}
	if (!lf_collectible(data) || !data->countcheck)
		return (0);
	return (1);
}
...
int	checklen(t_data *data)
{
	...
	{
		if ((ssize_t) ft_strlen(data->parsed_map[data->i]) != data->end)
			return (0);
		data->i++;
	}
	if (data->i == data->end)
